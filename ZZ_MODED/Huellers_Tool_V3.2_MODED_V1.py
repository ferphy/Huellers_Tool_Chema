...
def tech2freq(tech,zone):
    if zone == 'ZR':
        freqZR = {'S':700,'Q':700,'J':800,'G':900,'U':900,'K':1800,'M':2100,'W':2100,'L':2600,'P':3500}
        freq = np.zeros(len(tech))
        for i,t in enumerate(tech):
             freq[i] = freqZR[t]
    if zone == 'ZN':
        freqZN = {'Y':700,'Q':700,'M':800,'E':900,'F':900,'N':1800,'R':1800,'T':2100,'B':2100,'C':2100,'D':2100,'W':2100,'L':2600,'P':3500}
        freq = np.zeros(len(tech))
        for i,t in enumerate(tech):
            if t in freqZN.keys():
                freq[i] = freqZN[t]
...
def process_cellscoring_provincial():
    ...
    indexzona = (dfcellscoring.loc[:,'ZONA'] == 'Zona ESTE')|((dfcellscoring.loc[:,'ZONA'] == 'Zona SUR'))
...
def export_clusters():
    ...
    if okHuella.get():
        ...
        dictprov = {'V':'VAL','B':'BAL','U':'MUR','A':'AND','E':'EXT','R':'ARA','K':'CYM','X':'CLM'}
...
def calcdfbal(dfbal,cdfct,dfkpi4g,dateend,datestart):
    ...
    for i,ss in enumerate(sitesectors):
        ...
        dfbal.loc[i,'ENODEB'] = cdfct.loc[cdfct['Site-Sector'] == ss,'NODE'].tolist()[0]
        for j,b in enumerate(bands):
            ...
            if any(indexKPI & indexKPIdate):
                ...
                prbs = [float(x.replace(',', '.')) for x in dfkpi4g.loc[indexKPI & indexKPIdate,'PRB.DL.Usage.RATE(%)'].tolist() if (type(x) == str)&(x != '/0')&(x != '-')]
...
def process_PRB_cluster(cdfct,cname,seedsites,dfkpi4g,dfkpi5g,fname):
    ...
    datos_para_justificacion_folder = os.path.join(fname, cname, 'Datos_para_justificacion')
    os.makedirs(datos_para_justificacion_folder, exist_ok=True)


     # ========= NUEVA SECCIÓN: Verificar bandas disponibles por sitio =========
    def verificar_bandas_por_site(df, bandas_requeridas=['M', 'N', 'L', 'Y', 'T']):
        # Agrupar por 'SITE' y recopilar las bandas presentes
        bandas_por_site = df.groupby('SITE')['Band'].apply(set).reset_index()

        # Verificar qué bandas faltan
        bandas_por_site['Faltantes'] = bandas_por_site['Band'].apply(
            lambda x: list(set(bandas_requeridas) - x)
        )

        # Identificar si tiene todas las bandas
        bandas_por_site['Tiene_todas'] = bandas_por_site['Faltantes'].apply(
            lambda x: len(x) == 0
        )
        return bandas_por_site
    ...
    #============CODIGO DE FER==============
        # Extraer sitios y sectores degradados
    df_sites_degradados = dfbal.loc[indexbaldeg, ['SITE-SECTOR']]
    df_sites_degradados['Site'] = df_sites_degradados['SITE-SECTOR'].str[:7]
    df_sites_degradados['Sector'] = df_sites_degradados['SITE-SECTOR'].str[-1]
    ...
    #============CODIGO DE FER==============
        # Filtrar información de PRB degradados
    df_prb_degradados = dfprb4g.loc[indexprb4gdeg, ['EUTRANCELL', 'LB/HB', 'PRB HC', 'TH_HC', 'OK/NOK']]

    # Filtrar información de PDCCH degradados
    df_pdcch_degradados = dfprb4g.loc[indexpdcchdeg, ['EUTRANCELL', 'LB/HB', 'MAX PDCCH', 'OK/NOK']]

    # Asegúrate de que 'Band' y 'SITE' estén presentes en cdfct
    if 'Band' in cdfct.columns and 'SITE' in cdfct.columns:
        bandas_por_site = verificar_bandas_por_site(cdfct)
        # Guardar el resultado en Excel para análisis
        bandas_file = os.path.join(datos_para_justificacion_folder, f'Bandas_por_Site_{cname}.xlsx')
        bandas_por_site.to_excel(bandas_file, index=False)
        print(f"Archivo con bandas por sitio generado: {bandas_file}")
    else:
        print("No se encontraron columnas 'Band' o 'SITE' en el DataFrame.")

     # Verificar si hay resultados y guardar en Excel
    if not df_sites_degradados.empty:
        balance_file = os.path.join(datos_para_justificacion_folder, f'Balanceos_Degradados_{cname}.xlsx')
        df_sites_degradados.to_excel(balance_file, index=False)
        print(f"Archivo generado con sitios y sectores degradados: {balance_file}")
    else:
        print("No se encontraron sitios degradados.")


    # Guardar PRB degradados en Excel
    if not df_prb_degradados.empty:
        prb_file = os.path.join(datos_para_justificacion_folder, f'PRB_4G_Degradados_{cname}.xlsx')
        df_prb_degradados.to_excel(prb_file, index=False)
        print(f"Archivo generado con PRB degradados: {prb_file}")
    else:
        print("No se encontraron PRB degradados.")

    # Guardar PDCCH degradados en Excel
    if not df_pdcch_degradados.empty:
        pdcch_file = os.path.join(datos_para_justificacion_folder, f'PDCCH_4G_Degradados_{cname}.xlsx')
        df_pdcch_degradados.to_excel(pdcch_file, index=False)
        print(f"Archivo generado con PDCCH degradados: {pdcch_file}")
    else:
        print("No se encontraron PDCCH degradados.")
...
def process_Huella_cluster(cdfct,cname,dfkpi3g,dfkpi4g,dfkpih5g,fname,thorkeys,dfcellscoring,dfdb):
    ...
    cellnamexc = [x for x in cdfct.loc[icellnx,'CELLNAMEX'].tolist() if x[-1]=='A']
...
def tech2freq2(tech):
    tech2freqdict = {'Y':'700','M':'800','E':'900','F':'900','N':'1800','B':'2100','T':'2100','L':'2600','Q':'700','W':'2100','P':'3500'}
...
def complete_dfret(dfret):
    ...
    dictfreqZR = {'S':'700','Q':'700','J':'800','G':'900','U':'900','K':'1800','M':'2100','W':'2100','L':'2600'}
...
def query_bal_cluster_cosector(cdfct,cname,fname):
    ...
    for site in sites:
        ...
        if (region == 'Zona ESTE')|(region == 'Zona SUR'):
...
def process_power_ct(ct,cname,sitess,dfgtrxdev,dfucell,dfulocell,dfulocellsectoreqm,dfulocellcontr,dfupcpich,dfcell,dfpdschcfg,dfcelldlpcpdschpa,dfcellchpwrcfg,dfnrducelltrp,dfnrcell,dfnrducell):
    ...
    modpow4g = ''
    coln4g = ['site','nodo','neid','cellname','localcellid','dlbandwidth','antoutputpwr','referencesignalpwr','pb','papcoff','referencesignalpwr esperado','BANDA','ZONA','Check Diseño']
...
label_author = tkinter.Label(wd, 
    text="Developed by: José María Gómez Belmonte")
